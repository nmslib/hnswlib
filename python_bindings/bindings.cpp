...
public:
    ...
    py::object knnQuery_return_numpy(py::object input, py::object weights = py::none(), size_t k = 1, int num_threads = -1) {
...

        {
            ...
            if (!weights.is_none()) {
                ...
                if (w_buffer.shape[0] != features)
                    throw std::runtime_error("weights and features must have the same number of dimensions");
            }
            // avoid using threads when the number of searches is small:
            ...
            if(normalize==false) {
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
                                ...
                                if (weights.is_none())
                                    result = appr_alg->searchKnn((void *) items.data(row), k);
                                else
                                    result = appr_alg->searchKnn((void *) items.data(row), k, (void *) w_items.data());
                                if (result.size() != k)
                ...
                ParallelFor(0, rows, num_threads, [&](size_t row, size_t threadId) {
								...
								normalize_vector((float *) items.data(row), (norm_array.data()+start_idx));
                                if (weights.is_none())
                                    result = appr_alg->searchKnn((void *) (norm_array.data() + start_idx), k);
                                else
                                    result = appr_alg->searchKnn((void *) (norm_array.data() + start_idx), k, (void *) w_items.data());
                                if (result.size() != k)
                                    throw std::runtime_error(
                                            "Cannot return the results in a contigious 2D array. Probably ef or M is to small");
                                for (int i = k - 1; i >= 0; i--) {
...
PYBIND11_PLUGIN(hnswlib) {
        ...
        .def("knn_query", &Index<float>::knnQuery_return_numpy, py::arg("data"), py::arg("weights") = py::none(), py::arg("k")=1, py::arg("num_threads")=-1)
