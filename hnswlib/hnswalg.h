...
//                    if (candidate_id == 0) continue;
    ...
    void resizeIndex(size_t new_max_elements) {
        Status status = resizeIndexNoExceptions(new_max_elements);
        if (!status.ok()) {
            HNSWLIB_THROW_RUNTIME_ERROR(status.message());
        }
    }

    Status resizeIndexNoExceptions(size_t new_max_elements) {
    ...
    void markDelete(labeltype label) {
        Status status = markDeleteNoExceptions(label);
        if (!status.ok()) {
            HNSWLIB_THROW_RUNTIME_ERROR(status.message());
        }
    }
    ...
    */
    Status addPointNoExceptions(
        const void *data_point, labeltype label, bool replace_deleted = false)
        override {
        ...
        if (!is_vacant_place) {
            ...
            Status delete_status = unmarkDeletedInternal(internal_id_replaced);
            if (!delete_status.ok()) {
    ...
    Status updatePoint(const void *dataPoint, tableint internalId, float updateNeighborProbability) {
        ...
        return repairConnectionsForUpdate(dataPoint, entryPointCopy, internalId, elemLevel, maxLevelCopy);
    }
